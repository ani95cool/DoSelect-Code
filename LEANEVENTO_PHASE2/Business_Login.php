...
session_start();
   ...
   function sign_me_in($user, $password, $fecthed_email, $fetched_password, $con){
       $sql2 = "SELECT `Firstname` FROM `individual` WHERE `Email` = '$user' AND `Password` = '$password' ";
       $result2 = mysqli_query($con, $sql2);

       if(mysqli_num_rows($result2) > 0){
        $select = mysqli_fetch_assoc($result2);
        echo "<script>
        alert('Hey ".$select['Firstname']."');
   ...
   function Logmein($user, $password, $con){
      $sql = "SELECT `Email`, `Password` FROM `individual` WHERE `Email` = '$user' AND `Password` = '$password' ";
      print  $sql;
      ...
      if(mysqli_num_rows($result) > 0){
        $row = mysqli_fetch_assoc($result);
          ...
          if($row["Email"] == $user && $row["Password"] == $password){
            sign_me_in($user, $password, $fecthed_email, $fetched_password, $con);
          }else{
            print "Nothing found";
...
<body>
  ...
  <nav id="top_menu">
    <ul>
      ...
      <li></li>
      <li></li>
      <li class="new" style="color: yellow;">Inciar Sesion</li>
      <li></li>
      <li></li>
  ...
  <section id="main_section">
    ...
    <div class="larger">
    <form id="popup1" method="POST" action="<?php echo $_SERVER['PHP_SELF'];?>">
  <div class="popup">
  ...
  <div class="makemyday">
    <center><a class="lookme" style="font-size: 10px;color: #F1C40F;" href="">Olvido su contraseñaena?</a></center>

  </div><br>
      
  <div class="styleit">
      <center><input type="submit" id="bu1" class="form__submit button button_theme_dark">Registrarse</center>
    </div>
